...
echo $($PSQL "SELECT teams.name FROM teams JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY teams.name")
...
echo $($PSQL "SELECT DISTINCT games.year, teams.name FROM  teams JOIN games ON teams.team_id = games.winner_id WHERE games.round = 'Final'")

echo -e "\nList of teams that start with 'Co':"
echo $($PSQL "SELECT DISTINCT teams.name FROM teams JOIN games ON teams.team_id = games.winner_id OR teams.team_id = games.opponent_id WHERE teams.name LIKE 'Co%'")
